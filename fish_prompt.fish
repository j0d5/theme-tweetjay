function fish_prompt --description 'Write out the prompt'
  ...
  case '*'
    ...
    if test -z $current_user
        set promt_user_host (set_color $fish_color_user) $current_user $normal @ (set_color $fish_color_host) $__fish_prompt_hostname $normal
    end
  ...
  echo -n -s $prompt_user_host ' '
    ...
    echo -n -s $c1 (__jay_git_prompt) $normal
